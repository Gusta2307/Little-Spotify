...
import random
import sys
from utils import get_chord_node_instance, hashing
...
class ChordNode:
    ...
    @property
    def successor(self):
        return get_chord_node_instance(self._ft_node[1])
    ...
    def predecessor(self):
        return get_chord_node_instance(self._ft_node[0])
    ...
    def join(self, nodeid=None):
        ...
        # Is not the first node in the ring
            ...
            node = get_chord_node_instance(nodeid)
            try:
    ...
    def init_finger_table(self, node):
        ...
        for i in range(1, self.m):
            if self.inrange(self._ft_start[i+1], self.id, self._ft_node[i]):
                ...
                self._ft_node[i +
                              1] = node.find_successor(self._ft_start[i+1]).id
            ...
            try:
                if not self._successors_list:
                    ...
                    for i in range(len(self._successors_list)):
                        succ = get_chord_node_instance(
                            self._successors_list[i])
                        if succ is not None:
            ...
            if self.inrange(key, self._ft_node[0] + 1, self.id + 1):
                ...
                for k in range(1, self.m):
                    if self.inrange(key, self._ft_start[k], self._ft_start[k + 1]):
                        node = get_chord_node_instance(self._ft_node[k])
                        if node is not None:
                            return node.lookup(key)
                else:
                    node = get_chord_node_instance(self._ft_node[self.m])
                    if node is not None:
                        return node.lookup(key)
